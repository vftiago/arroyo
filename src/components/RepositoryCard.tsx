...
const RepositoryCard = ({ repositoryNode }: RepositoryCardProps) => {
  ...
  return (
    <GlassPanel customStyles="flex flex-col justify-between gap-4 p-4 flex-1 min-h-64">
      <div className="flex items-center justify-between border-b border-neutral-500/20 pb-2">
        <h4 className="text-base font-bold">{name}</h4>

        {homepageUrl && (
          <a className="text-sm" target="_blank" rel="noreferrer" href={homepageUrl}>
            {homepageUrl}
          </a>
        )}
      </div>
      ...
      <div className="flex">
        <a className="text-sm text-neutral-300" target="_blank" rel="noreferrer" href={url}>
          {url}
        </a>
      </div>
    </GlassPanel>
